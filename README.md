...
`app/api/og/route.tsx`가 `next/og`로 1200x630 이미지를 그려서 카카오톡/트위터 등 공유 미리보기에 쓴다.
`/api/og?a=<encoded>`는 해당 결과의 팀 이름·궁합%·한줄평을 렌더링하고, `a` 없이 호출하면 랜딩용 기본 카드를 그린다.
`app/layout.tsx`(기본 OG)와 `app/result/page.tsx`의 `generateMetadata`(결과별 OG)에서 각각 참조한다.
한글 렌더링을 위해 Pretendard TTF를 jsDelivr(npm 패키지 배포본)에서 요청마다 fetch한다.
